...
function formatArmy(army: Army | null): string {
  if (!army) return '无'
  const parts = [`${army.troops}兵`]
  if (army.marchDaysLeft) parts.push(`行军${army.marchDaysLeft}天`)
  if (army.inCombat) parts.push(`战斗${army.combatDaysLeft ?? 0}天`)
  return parts.join(' · ')
}
...
export function buildGameSnapshot(ctx: GameReportContext): string[] {
  ...
  for (const [fid, faction] of Object.entries(save.factions)) {
    ...
    lines.push(
      `${fid}: 粮=${faction.food.toFixed(1)} 军=${faction.armies.length}支/${troops}兵 策=${faction.policies.length}`,
    )
  }
...
export function formatTileSelect(info: TileSelectInfo): string[] {
  ...
  return [
    `选中 ${tile.name}(${tile.gridX},${tile.gridY}) 归属=${owner}${army ? ` 兵${army.troops}` : ''}`,
