...
> **本文档**：编制、UI、图层、战斗经济的细致愿景 + 分 Phase 交付路线。
...
| 千人队 | **是** | 番号 + 兵力 | 行军、战斗、编入将军队 |
| 将军队 | 是（旗） | 军旗 + 汇总兵力 | 任命将军、收纳千人队 |
...
| 迁移 | 1 旧 `Army` → 1 `Corps` + 1 队 + 10 百人队 |
...
src/
  ...
  core/map-layers/       # military, terrain, resource renderers
  ui/shell/              # top-bar, bottom-bar, layout
  ui/minimap.ts
  ui/modal-host.ts
  ui/corps-bar.ts        # 将领栏：新编、待命、换行增高
  ui/corps-detail.ts     # 军队详情 + 将领任命
  ui/battalion-detail.ts # 编队详情
...
- `buildArmyDisplay` → 按番号绘制；将军队旗单独 overlay。
...
**下一开发起点**：v0.7（布局 + 小地图 + 图层 + 弹窗框架 + 将领栏静态原型）。
...
*文档版本：2026-06-17 rev.2 · 基线 v0.6.0*
